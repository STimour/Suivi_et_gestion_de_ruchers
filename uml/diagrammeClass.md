classDiagram
    ...
    class EntrepriseProfile {
        +id: UUID
        +typeProfile: TypeProfileEntreprise
        +created_at: DateTime
        +updated_at: DateTime
    }
    ...
    class TypeProfileEntreprise {
        ApiculteurProducteur
        EleveurDeReines
        Pollinisateur
    }
    ...
    class CycleElevageReine {
        +id: UUID
        +racleId: UUID
        +dateDebut: Date
        +dateFin: Date?
        +statut: StatutCycleElevage
        +created_at: DateTime
        +updated_at: DateTime
    }

    class StatutCycleElevage {
        EnCours
        Termine
        Annule
    }

    class TacheCycleElevage {
        +id: UUID
        +cycleId: UUID
        +type: TypeTacheElevage
        +jourTheorique: Integer
        +datePrevue: Date
        +dateRealisee: Date?
        +statut: StatutTacheElevage
        +commentaire: Text?
        +created_at: DateTime
        +updated_at: DateTime
    }

    class TypeTacheElevage {
        Greffage
        Operculation
        NaissanceReine
        ControleVolFecondation
        ValidationPonte
        MarquageReine
        MiseEnVente
    }

    class StatutTacheElevage {
        AFaire
        Faite
        EnRetard
        Annulee
    ...
    %% ===== Relations mise à jour =====
    Reine "1" --> "0..1" RacleElevage : eleveSur
    RacleElevage "1" --> "0..*" CycleElevageReine : cycle
    CycleElevageReine "1" --> "1..*" TacheCycleElevage : planifie
    ...
    <<enumeration>> TypeProfileEntreprise
    <<enumeration>> ReineStatut
    <<enumeration>> StatutCycleElevage
    <<enumeration>> TypeTacheElevage
    <<enumeration>> StatutTacheElevage
    ...
    Entreprise "1" --> "0..*" EntrepriseProfile : profils
    LimitationOffre "1" --> "0..*" Offre : appliqueParDefaut
    ...
    class LimitationOffre:::mainclass
    class Utilisateur:::mainclass
    class Entreprise:::mainclass
    class EntrepriseProfile:::mainclass
    ...
    class RacleElevage:::mainclass
    class CycleElevageReine:::mainclass
    class TacheCycleElevage:::mainclass
    ...
    class TypeProfileEntreprise:::enumclass
    class ReineStatut:::enumclass
    class StatutCycleElevage:::enumclass
    class TypeTacheElevage:::enumclass
    class StatutTacheElevage:::enumclass
